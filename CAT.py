...
final_catboost = CatBoostClassifier(**params)

# Train the model on the balanced data
print('Training the final model on balanced data...')
final_catboost.fit(X_train_balanced, y_train_balanced, eval_set=(X_test_scaled, y_test), verbose=100)

# Evaluate the final model on the test set
print('\nEvaluating the final model on the test set...')
y_test_pred = final_catboost.predict(X_test_scaled)
...
# Calculate ROC AUC
y_test_pred_proba = final_catboost.predict_proba(X_test_scaled)[:, 1]
...
model_filename = 'best_catboost_model_smote.pkl'
scaler_filename = 'scaler_smote.pkl'

with open(model_filename, 'wb') as file:
    pickle.dump(final_catboost, file)
